...
for location in locations:
  ...
  for i in range(n):
    temp.append(arr['rows'][0]['elements'][i]['duration']['value'])
  gmaps_distance_matrix.append(temp)
#print(gmaps_distance_matrix)

m = sum(people_count)
duration_matrix = np.zeros(shape=(m, m), dtype=int)
  ...
  for j2 in range(n):
    for i3 in range(i1, i1+people_count[i2]):
      for j3 in range(j1, j1+people_count[j2]):
        duration_matrix[i3][j3] = gmaps_distance_matrix[i2][j2]
    j1 += people_count[j2]
  i1 += people_count[i2]
...
for i in range(n):
  for j in range(ind, ind+people_count[i]):
    distance_to_office[j] = gmaps_distance_to_office['rows'][i]['elements'][0]['duration']['value']
  ind += people_count[i]
